...
class UserRepo {
    ...
    static login(email) {
        return new Promise((resolve, reject) => {
            db_1.default.connection.query(`SELECT password , more_secured ,id FROM users WHERE email = ?`, [email], (err, res) => {
                if (err) {
                    resolve(undefined);
                }
                else {
    ...
    static getVCode({ email }) {
        return new Promise((resolve, reject) => {
            db_1.default.connection.query(`SELECT code, id FROM users WHERE email = ?`, [email], (err, res) => {
                if (err)
                    resolve(false);
                else
                    resolve(res);
