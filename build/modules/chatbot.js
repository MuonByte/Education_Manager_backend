...
function createRoom_Handler(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!roomName || typeof roomName !== "string") {
            return res.status(400).json({ error: "roomName is required or type is not correct" });
        }
        let token;
        ...
        try {
            const newRoom = yield (0, chatbot_1.createNewRoom)(roomName);
            console.log("newRoom is:", newRoom); //----------
            const simpleMessage = "HELLO how can i help u today";
            const newMessage = yield (0, message_1.saveAIMessage)(newRoom, simpleMessage);
...
function listChatbotRooms_Handler(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let token;
        if (authHeader && authHeader.startsWith("Bearer ")) {
            token = authHeader.split(" ")[1]; // ده اللي هتستخدمه فعليًا
        }
        console.log(token);
        if (!token || typeof token !== "string")
            return res.status(400).json({ error: "Token is required or type is not correct" });
...
function renameChatbotRoom_Handler(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const payload = jsonwebtoken_1.default.verify(token, secret);
        const id = payload.id;
        if (!id || typeof id !== "string" || !roomName || typeof roomName !== "string") {
            return res.status(400).json({ error: "ID or type is not correct roomName are required or type is not correct " });
        }
        try {
            const result = yield (0, chatbot_1.renameChatbotRoom)(id, roomName);
            console.log(result); //-----------------------
            if (result) {
...
function deleteChatRoom(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const result = yield (0, chatbot_1.deleteChatbotRoom)(roomId);
            console.log(result); //-----------------------
            if (result) {
                return res.status(200).json({ message: "Chatbot room deleted successfully" });
            }
            else {
                return res.status(404).json({ error: "Chatbot room not found" });
            }
            ...
            console.error("Error deleting chatbot room:", error);
            return res.status(500).json({ error: " server error" });
        }
    });
}
