...
class DocumentRepo {
    static getDocumentsByTimestamp(id) {
        return new Promise((resolve, reject) => {
            databaseconfig_1.default.connection.query("SELECT * FROM document WHERE chatbot_rooms_id = ? ORDER BY created_at ASC", [id], (error, results) => {
                if (error) {
    ...
    static getDocumentsUrl(id) {
        return new Promise((resolve, reject) => {
            databaseconfig_1.default.connection.query("SELECT file_url FROM documents WHERE chatbots_room_id = ?", [id], (error, results) => {
                if (error) {
    ...
    static seveResultDocument(document) {
        return new Promise((resolve, reject) => {
            ...
            databaseconfig_1.default.connection.execute(`INSERT INTO documents (id ,chatbot_rooms_id, users_id, name, file_url, original, augmented) VALUES (?, ?, ?, ?, ?, ?,?) `, [document.id, document.chatbot_rooms_id, document.users_id, document.name, document.file_url, document.original, document.augmented], (error) => {
                if (error) {
    ...
    static getDocumentById(id) {
        return new Promise((resolve, reject) => {
            databaseconfig_1.default.connection.query("SELECT * FROM documents WHERE id = ?", [id], (error, results) => {
                if (error) {
