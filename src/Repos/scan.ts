...
export class DocumentRepo{
  ...
  static getDocumentsByTimestamp(id: string):Promise<Documents[]|undefined> {
    return new Promise((resolve, reject) => {
      DatabaseConfig.connection.query("SELECT * FROM document WHERE chatbot_rooms_id = ? ORDER BY created_at ASC", [id], (error, results: Documents[]) => {
        if (error) {
  ...
  static getDocumentsUrl(id:string):Promise<String|undefined> {
    return new Promise((resolve, reject) => {
      DatabaseConfig.connection.query("SELECT file_url FROM documents WHERE chatbots_room_id = ?", [id], (error, results: Documents[]) => {
        if (error) {
  ...
  static seveResultDocument(document:Documents):Promise<boolean|undefined> {
    return new Promise((resolve, reject) => {
      ...
      DatabaseConfig.connection.execute(`INSERT INTO documents (id ,chatbot_rooms_id, users_id, name, file_url, original, augmented) VALUES (?, ?, ?, ?, ?, ?,?) `, [document.id,document.chatbot_rooms_id, document.users_id, document.name, document.file_url, document.original, document.augmented], (error) => {
        if (error) {
  ...
  static getDocumentById(id: string):Promise<Documents|undefined> {
    return new Promise((resolve, reject) => {
      DatabaseConfig.connection.query("SELECT * FROM documents WHERE id = ?", [id], (error, results: Documents[]) => {
        if (error) {
